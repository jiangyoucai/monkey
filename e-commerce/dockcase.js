...
(function () {
  ...
  const getMedia = () => {
    ...
    console.log("-----------thumb-----------");
    const thumbList = document.getElementsByClassName("product-gallery__media");
    ...
    console.log("-----------detail-----------");
    const richList = document.getElementsByClassName("shopify-section");

    let id = 0;
    for (let i = 0; i < richList.length; i++) {
      const className = richList[i].className;

      if (
        className !== "shopify-section shopify-section--single-image" &&
        className !== "shopify-section shopify-section--media-with-swiper"
      ) {
        continue;
      }
      ...
      for (let index = 0; index < tmp.length; index++) {
        ...
        if (source && isURL(source)) {
          const path = getPath(source);
          const name = getFile("detail", id, getExtension(path));
